...
grep -q "^  doctor json " "$tmp_dir/help.out"
grep -q "^  doctor env " "$tmp_dir/help.out"
grep -q "^  doctor env json " "$tmp_dir/help.out"
grep -q "^  doctor framework " "$tmp_dir/help.out"
grep -q "^  doctor framework json " "$tmp_dir/help.out"
grep -q "^  doctor runtime " "$tmp_dir/help.out"
grep -q "^  doctor runtime json " "$tmp_dir/help.out"
...
grep -q "https://carbide.ryangerardwilson.com/for/agents" "$tmp_dir/demo/AGENTS.md"
grep -q "https://raw.githubusercontent.com/ryangerardwilson/carbide/main/docs/site/for/agents.md" "$tmp_dir/demo/AGENTS.md"
grep -q "intentionally does not include .*\`agents.d/\`" "$tmp_dir/demo/AGENTS.md"
grep -q "PROJECT.md" "$tmp_dir/demo/AGENTS.md"
grep -q "carbide run dev" "$tmp_dir/demo/AGENTS.md"
grep -q "carbide doctor" "$tmp_dir/demo/AGENTS.md"
grep -q "carbide status" "$tmp_dir/demo/AGENTS.md"
grep -q "## Product Truth" "$tmp_dir/demo/PROJECT.md"
grep -q "## Users And Roles" "$tmp_dir/demo/PROJECT.md"
grep -q "## Business Rules" "$tmp_dir/demo/PROJECT.md"
grep -q "## Acceptance Criteria" "$tmp_dir/demo/PROJECT.md"
...
grep -Eq "^project shape[[:space:]]+ok[[:space:]]+web api db" "$tmp_dir/doctor.out"
...
grep -Eq "^agents[[:space:]]+ok[[:space:]]+AGENTS.md /for/agents" "$tmp_dir/doctor.out"
grep -Eq "^product context[[:space:]]+ok[[:space:]]+PROJECT.md" "$tmp_dir/doctor.out"
...
CARBIDE_HOME="$repo_root" "$repo_root/cli/bin/carbide" doctor json > "$tmp_dir/doctor.json"
grep -q '"command": "doctor"' "$tmp_dir/doctor.json"
grep -q '"ok": true' "$tmp_dir/doctor.json"
grep -q '"check": "product context"' "$tmp_dir/doctor.json"

CARBIDE_HOME="$repo_root" "$repo_root/cli/bin/carbide" doctor env json > "$tmp_dir/doctor-env.json"
grep -q '"command": "doctor env"' "$tmp_dir/doctor-env.json"
grep -q '"status": "ok"' "$tmp_dir/doctor-env.json"
