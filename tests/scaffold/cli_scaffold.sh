...
grep -q "^  deploy prod " "$tmp_dir/help.out"
...
! grep -q "^\\[deploy\\]" "$tmp_dir/demo/carbide.toml"
...
grep -Eq "^deploy targets[[:space:]]+ok[[:space:]]+0 checked-in scripts" "$tmp_dir/health.out"
...
grep -Eq "^line limits[[:space:]]+ok[[:space:]]+all checked files <= 1000 lines" "$tmp_dir/health.out"
...
if CARBIDE_HOME="$repo_root" "$repo_root/cli/bin/carbide" deploy prod > "$tmp_dir/deploy.out" 2> "$tmp_dir/deploy.err"; then
  printf 'carbide deploy prod should fail until a target exists\n' >&2
  exit 1
fi
grep -q "no checked-in deploy target named prod" "$tmp_dir/deploy.err"
