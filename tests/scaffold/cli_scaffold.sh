...
grep -q "^  project migrate " "$tmp_dir/help.out"
...
grep -q "web/src/lib/cx.ts" "$tmp_dir/demo/agents.d/TAILWIND_COMPONENTS.md"
...
grep -q "Custom class selectors" "$tmp_dir/demo/agents.d/TAILWIND_COMPONENTS.md"
...
grep -q "bg-carbide-hero text-carbide-hero-text" "$tmp_dir/demo/web/src/component/l1/tokens.ts"
...
! grep -Eq '^[[:space:]]*#[A-Za-z_-]' "$tmp_dir/demo/web/src/styles.css"
! grep -Eq '@apply|@layer|@keyframes|@media|@container|@plugin|@config' "$tmp_dir/demo/web/src/styles.css"
...
grep -Eq "^frontend[[:space:]]+ok[[:space:]]+Bun React Tailwind TypeScript" "$tmp_dir/doctor.out"
