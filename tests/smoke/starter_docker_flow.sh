...
cleanup() {
  if [ -n "$port" ] && [ -d "$tmp_dir/demo" ]; then
    (
      cd "$tmp_dir/demo"
      CARBIDE_HTTP_PORT="$port" docker compose -f "$compose_file" down -v --remove-orphans >/dev/null 2>&1 || true
    )
  fi
  rm -rf "$tmp_dir"
}
...
cd "$tmp_dir"
"$repo_root/cli/bin/carbide" new demo >/dev/null

cd "$tmp_dir/demo"
docker compose -f "$compose_file" config > "$tmp_dir/compose.config"
grep -q "develop:" "$tmp_dir/compose.config"
grep -q "watch:" "$tmp_dir/compose.config"
grep -q "action: rebuild" "$tmp_dir/compose.config"
grep -q "/web/src" "$tmp_dir/compose.config"
grep -q "/web/package.json" "$tmp_dir/compose.config"
grep -q "/web/bun.lock" "$tmp_dir/compose.config"
grep -q "/api" "$tmp_dir/compose.config"
grep -q "/db" "$tmp_dir/compose.config"
grep -q "api/Dockerfile" "$tmp_dir/compose.config"
CARBIDE_HTTP_PORT="$port" docker compose -f "$compose_file" up -d --build
...
grep -q "api listening on container port 8080" "$tmp_dir/api.log"
grep -q "public API URL is http://localhost:$port/api" "$tmp_dir/api.log"
docker compose -f "$compose_file" logs web > "$tmp_dir/web.log"
grep -q "Carbide Bun frontend listening inside container on :8080" "$tmp_dir/web.log"
grep -q "browser entrypoint http://localhost:$port" "$tmp_dir/web.log"
grep -q "proxying /api and /health to api service http://api:8080" "$tmp_dir/web.log"
! grep -q "Bun frontend listening on http://localhost:8080" "$tmp_dir/web.log"
CARBIDE_HOME="$repo_root" "$repo_root/cli/bin/carbide" status > "$tmp_dir/status.out"
...
grep -Eq "^web[[:space:]]+demo-web-1[[:space:]]+localhost:$port[[:space:]]+8080/tcp[[:space:]]+running" "$tmp_dir/status.out"
...
if ! docker compose -f "$compose_file" run --rm --no-deps web bun run build > "$tmp_dir/web-build.log" 2>&1; then
