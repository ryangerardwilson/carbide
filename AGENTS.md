# Carbide Agent Guide

This file is for agents and maintainers iterating on the Carbide framework
repo. The public README is for humans evaluating and installing Carbide; do not
turn it into a roadmap, scratchpad, or internal operating manual.

## Source Of Truth

Framework repo work uses this file plus the narrow engineering contract under
`docs/engineering/`. The `/for/agents` route is not the framework repo's own
operating guide.

- Product promise and README boundary: `docs/engineering/PRODUCT_CONTRACT.md`
- Eternal product laws: `docs/engineering/LAWS.md`
- Current starter taste: `docs/engineering/TASTE_GUIDE.md`
- Repo layout and ownership: `docs/engineering/REPO_STRUCTURE.md`
- CLI and versioning rules: `docs/engineering/CLI_AND_VERSIONING.md`
- Generated app contract: `docs/engineering/SCAFFOLD_CONTRACT.md`
- Documentation app contract: `docs/engineering/DOCS_APP.md`
- Regression checks: `docs/engineering/REGRESSION_CHECKS.md`
- Roadmap and future work: `docs/engineering/ROADMAP.md`

Read the smallest relevant file before editing. Do not load every file by
default.

## Scaffolded App Agents

`https://carbide.ryangerardwilson.com/for/agents` is the startup guide for
agents working inside apps generated by `carbide new` or `carbide init`.
Its checked-in source is `docs/site/for/agents.md`.

Generated apps start without local `AGENTS.md` or `README.md`. The framework
repo root `AGENTS.md` should not be replaced by, or reduced to, the
generated-app agent startup guide. In generated apps, `/for/agents` owns
framework guidance. If an app owner later adds local `AGENTS.md` or
`README.md`, those files are app-owned context rather than framework-owned
contract files. Carbide itself never rewrites existing app code; when an audit
leads to app-code changes, those are Codex edits made intentionally inside the
app, not framework-owned rewrites.

## Hard Rules

- Keep Carbide monorepo-first. Containers define runtime boundaries; they do
  not justify default microservice sprawl.
- Keep the generated app Docker-first: `web`, `api`, and `db` are separate
  containers in one app repo.
- Keep README human-first: what Carbide is, what the user gets, how to install,
  and where to read more.
- Put framework iteration rules, roadmap phases, internal contracts, and
  regression details in `AGENTS.md` or `docs/engineering/`.
- Keep root directories intentional. The framework repo root should stay small:
  `cli`, `docs`, `scaffold`, and `tests`.
- Do not add root-level generated app files. Generated-app examples belong in
  `scaffold/`; docs-app runtime belongs in `docs/app/`.
- Before changing scaffold output, update contract tests and run the scaffold
  checks.

## Normal Verification

Use the narrowest check that proves the change, then run broader checks before
shipping contract changes:

```sh
cd cli && go test ./...
bash tests/contract/check_repo_contract.sh
PATH=/home/ryan/.local/share/mise/installs/go/1.26.4/bin:$PATH bash tests/scaffold/cli_scaffold.sh
carbide health framework
```

For docs app changes:

```sh
cd docs/app/web && bun run typecheck && bun run assets:build
cd docs/app && docker compose build web
```

For scaffold web changes:

```sh
cd scaffold/web && bun install --frozen-lockfile && bun run typecheck && bun run assets:build
```
