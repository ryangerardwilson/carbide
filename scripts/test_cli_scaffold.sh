...
test -f "$tmp_dir/demo/frontend/Dockerfile"
test -f "$tmp_dir/demo/frontend/index.html"
test -f "$tmp_dir/demo/frontend/package.json"
test -f "$tmp_dir/demo/frontend/bun.lock"
test -f "$tmp_dir/demo/frontend/src/main.jsx"
test -f "$tmp_dir/demo/frontend/src/server.jsx"
test -f "$tmp_dir/demo/frontend/src/styles.css"
! test -f "$tmp_dir/demo/frontend/package-lock.json"
! test -f "$tmp_dir/demo/frontend/vite.config.js"
...
grep -q "path: ./frontend/src" "$tmp_dir/demo/docker-compose.yml"
grep -q "path: ./frontend/package.json" "$tmp_dir/demo/docker-compose.yml"
grep -q "path: ./frontend/bun.lock" "$tmp_dir/demo/docker-compose.yml"
...
grep -q "oven/bun:1.3.14-debian" "$tmp_dir/demo/frontend/Dockerfile"
grep -q "bun install --frozen-lockfile" "$tmp_dir/demo/frontend/Dockerfile"
grep -q '"@tailwindcss/cli": "4.3.2"' "$tmp_dir/demo/frontend/package.json"
grep -q '"tailwindcss": "4.3.2"' "$tmp_dir/demo/frontend/package.json"
grep -q '"react": "19.2.7"' "$tmp_dir/demo/frontend/package.json"
grep -q "Bun.serve" "$tmp_dir/demo/frontend/src/server.jsx"
grep -q "proxying /api and /health" "$tmp_dir/demo/frontend/src/server.jsx"
grep -q '@import "tailwindcss";' "$tmp_dir/demo/frontend/src/styles.css"
grep -q '/api/${mode}' "$tmp_dir/demo/frontend/src/main.jsx"
grep -q "Bun frontend + C API + Postgres" "$tmp_dir/demo/frontend/src/main.jsx"
grep -q "React + Bun container" "$tmp_dir/demo/frontend/src/main.jsx"
grep -q "Containerized full stack development" "$tmp_dir/demo/frontend/src/main.jsx"
! find "$tmp_dir/demo" -path '*/view/*' -print -quit | grep -q .
