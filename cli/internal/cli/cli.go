...
func healthDocsAgentsContract() healthResult {
	...
	required := map[string][]string{
		...
		"../../docs/engineering/DOCS_APP.md": {
			...
			"preserve the Carbide docs brand palette",
			"black/yellow family across modes",
			"CARBIDE_DOCS_DEPLOY_SSH",
			"carbide deploy apply prod",
		},
	}
...
func projectAuditBrief(projectPath string, name string, slug string, scaffoldPath string, home string) string {
	rows := []string{
		...
		"## Laws",
		"",
		"Use `Law 1` through `Law 7` when reporting compliance:",
		"",
		"### Law 1. One App Repo",
		"",
		...
		"",
		"Browser flow stays same-origin: `web -> /api -> api`.",
		"",
		...
		"## Current Taste",
		"",
		"Use `Taste 1` through `Taste 6` when reporting starter alignment:",
		"",
		"### Taste 1. Starter Stack",
		"",
		"The starter reference shows Carbide's current preferred stack.",
		"",
		"### Taste 2. File Layout Details",
		"",
		"The starter reference shows Carbide's current file layout details.",
		"",
		"### Taste 3. UI Defaults",
		"",
		"The starter reference shows Carbide's current UI defaults and starter auth/dashboard shape.",
		"",
		"### Taste 4. Runtime Pins",
		"",
		"The starter reference shows Carbide's current runtime pins.",
		"",
		"### Taste 5. Docs And Examples",
		"",
		"The starter reference shows Carbide's current docs wording and example flows.",
		"",
		"### Taste 6. App Ownership",
		"",
		...
		"3. Report findings with `Law N` and `Taste N` references where useful.",
		"4. Carbide itself never rewrites app files. If the user wants changes, Codex may edit app code intentionally during the audit.",
		"5. Preserve local product docs such as `README.md` or `AGENTS.md` when they exist, plus local behavior, deploy targets, secrets, and data unless the user explicitly asks to change them.",
		"6. Expect manual judgment. This command does not merge conflicts, move business logic, or rewrite the current app for you.",
		"7. Do not copy generated output such as `node_modules`, `web/public`, `web/src/tailwind.css`, `.carbide`, or local `.env` files.",
		"8. Run `carbide health runtime` when runtime behavior, auth, or containers changed, plus the app-specific build/test commands.",
		"",
		"## Suggested Prompt For Codex",
		"",
		"```text",
		"Use .carbide/audit/*/starter-reference as the current Carbide starter reference, not as framework-managed truth. Fix any carbide health law failures first. Then compare the app to current Carbide taste, report findings by Law N and Taste N where useful, propose intentional changes, and edit app code only when the user wants those changes. Finish by running carbide health and the relevant build/tests.",
		"```",
		"",
	}
...
func auditCodexPrompt(auditRoot string, briefPath string, scaffoldPath string) string {
	rows := []string{
		...
		"3. Report findings with `Law N` and `Taste N` references where useful.",
		"4. Preserve local product docs such as `README.md` or `AGENTS.md` when they exist, plus local behavior, deploy targets, secrets, and data unless the user explicitly asked to change them.",
		"5. Carbide itself does not own existing app code after scaffold. If changes are appropriate, edit app code intentionally during the audit.",
		"6. Do not copy generated output such as `node_modules`, `web/public`, `web/src/tailwind.css`, `.carbide`, or local `.env` files.",
		"7. Finish with `carbide health`, `carbide health runtime` when runtime behavior changed, plus the relevant app build/test commands.",
		"",
